...
public class DefaultServiceRouteBuilder<T> extends CommonRouteBuilder {
	...
	public void bindService() throws Exception {
		...
		if (!Optional.ofNullable(proxyServiceAnnotation).isPresent()) {
			throw new InvalidServiceCandidateException("Service candidates must be anotated with com.accenture.service.ProxyService annotation.");
		}
		...
		for (Method method : serviceMethods) {
			serviceChoiceDefinition
				.when(header("method").isEqualTo(method.getName()))
					.bean(instance, method.getName());
			
		}
		
		serviceChoiceDefinition.otherwise()
			.process(exchange -> {
